...
router.post('/register', async (req, res) => {
    try {
        ...
        // Create new user (Note: Later we will add bcrypt hashing here for security)
...
router.post('/login', async (req, res) => {
    try {
        ...
        res.status(200).json({ message: `Welcome back, ${user.username}!`, walletBalance: user.walletBalance });
    } catch (error) {
